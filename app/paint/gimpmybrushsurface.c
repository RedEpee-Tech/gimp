...
static inline float
calculate_rr (int xp, int yp, float x, float y, float aspect_ratio, float sn,
...
calculate_dab_roi (float x, float y, float radius)
{
...
static void
gimp_mypaint_surface_get_color_2 (MyPaintSurface2 *base_surface, gfloat x, gfloat y,
                                  gfloat radius, gfloat *color_r, gfloat *color_g,
                                  gfloat *color_b, gfloat *color_a, gfloat paint)
{
...
static void
gimp_mypaint_surface_get_color_wrapper (MyPaintSurface *surface, float x, float y,
                                        ...
                                        float *color_b, float *color_a)
{
  return gimp_mypaint_surface_get_color_2 ((MyPaintSurface2 *) surface, x, y, radius,
                                           color_r, color_g, color_b, color_a, -1.0);
}

static gint
gimp_mypaint_surface_draw_dab_2 (
    MyPaintSurface2 *base_surface, gfloat x, gfloat y, gfloat radius,
    gfloat color_r, gfloat color_g, gfloat color_b, gfloat opaque, gfloat hardness,
    gfloat color_a, gfloat aspect_ratio, gfloat angle, gfloat lock_alpha,
    gfloat colorize, gfloat posterize, gfloat posterize_num, gfloat paint)
{
  GimpMybrushSurface *surface = (GimpMybrushSurface *) base_surface;
  ...
  const Babl         *rgb_to_hsl_fish = babl_fish (babl_format ("R'G'B' float"),
                                                   babl_format ("HSL float"));
  const Babl         *hsl_to_rgb_fish = babl_fish (babl_format ("HSL float"),
  ...
  if (surface->paint_mask)
    {
      ...
      mask_roi.y -= surface->paint_mask_y;
      gegl_buffer_iterator_add (iter, surface->paint_mask, &mask_roi, 0,
                                babl_format ("Y float"), GEGL_ACCESS_READ, GEGL_ABYSS_NONE);
    }
           ...
           iy < iter->items[0].roi.y + iter->items[0].roi.height; iy++)
        {
          for (ix = iter->items[0].roi.x;
               ix < iter->items[0].roi.x + iter->items[0].roi.width; ix++)
            {
              ...
              if (radius < 3.0f)
                rr = calculate_rr_antialiased (ix, iy, x, y, aspect_ratio, sn,
...
static int
gimp_mypaint_surface_draw_dab_wrapper (
    MyPaintSurface *surface, float x, float y, float radius, float color_r,
...
static void
gimp_mypaint_surface_end_atomic_2 (MyPaintSurface2 *base_surface, MyPaintRectangles *rois)
{
  GimpMybrushSurface *surface = (GimpMybrushSurface *) base_surface;
...
static void
gimp_mypaint_surface_end_atomic_wrapper (MyPaintSurface *surface, MyPaintRectangle *roi)
{
...
void
gimp_mypaint_surface_set_buffer (GimpMybrushSurface *surface, GeglBuffer *buffer,
                                 gint paint_mask_x, gint paint_mask_y)
{
...
void
gimp_mypaint_surface_set_offset (GimpMybrushSurface *surface, gint off_x, gint off_y)
{
  surface->off_x = off_x;
  surface->off_y = off_y;
}
...
void
gimp_mypaint_surface_get_offset (GimpMybrushSurface *surface, gint *off_x, gint *off_y)
{
  *off_x = surface->off_x;
  *off_y = surface->off_y;
}
